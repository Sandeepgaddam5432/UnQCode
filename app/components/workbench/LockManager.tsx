            ...
            {filteredAndSortedItems.map((item) => (
              <li
                ...
                className={classNames(
                  'text-unqcode-elements-textTertiary flex items-center gap-2 px-2 py-1 rounded hover:bg-unqcode-elements-background-depth-2 transition-colors group',
                  selectedItems.has(item.path) ? 'bg-unqcode-elements-background-depth-2' : '',
                )}
              >
                <Checkbox
                  checked={selectedItems.has(item.path)}
                  onCheckedChange={() => handleSelectItem(item.path)}
                  className="w-3 h-3 rounded border-unqcode-elements-borderColor"
                  aria-labelledby={`item-label-${item.path}`} // For accessibility
                />
                <span
                  className={classNames(
                    'shrink-0 text-unqcode-elements-textTertiary text-xs',
